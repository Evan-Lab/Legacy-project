// Welcome page tests (loads GeneWeb index page)
describe('Welcome page (server)', () => {
  const base = Cypress.env('BASE_URL') || Cypress.config('baseUrl') || 'http://localhost:2317';
  ...
  it('lists many language buttons with Bootstrap classes', () => {
    // Verify English button
    cy.get('a[title="en"]')
      .should('contain.text', 'English')
      .and('have.class', 'btn')
      .and('have.class', 'btn-outline-secondary');

    // Verify French button
    cy.get('a[title="fr"]')
      .should('contain.text', 'français')
      .and('have.class', 'btn')
      .and('have.class', 'btn-outline-secondary');

    // Verify German button
    cy.get('a[title="de"]')
      .should('contain.text', 'Deutsch')
      .and('have.class', 'btn')
      .and('have.class', 'btn-outline-secondary');

    // Verify Spanish button
    cy.get('a[title="es"]')
