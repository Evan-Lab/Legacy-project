...
@app.get("/{base}/home", response_class=HTMLResponse)
async def home(request: Request, book: BookAppDep) -> HTMLResponse:
    ...
    def url_set(*args):
        ...
        keys = args[:-1]
        value = args[-1]
        params = "&".join(f"{k}={value}" for k in keys)
        return f"/gw?{params}"
    ...
    random_helper = type(
        ...
        {
            "init": "",
            "get": lambda self, n: random_module.randint(0, n - 1) if n > 0 else 0,
        },
    )()
    ...
    return templates.TemplateResponse("home.html.j2", ctx)
